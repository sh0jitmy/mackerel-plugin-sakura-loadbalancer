...
- **`version.go` の保護 (Preservation of version.go)**:
  - リポジトリのルート直下にある `version.go` は自動タグ付けおよびリリースツール `tagpr` が機能するために必須のファイルです。このファイルを他のディレクトリ（例：`cmd/` 配下など）へ移動したり、手動で直接バージョン文字列などを編集したりしてはなりません。GoのCLI側でバージョンを参照したい場合は、`cmd/` 内の `main.go` などでローカルに定数を定義するか、ビルド時に埋め込むようにしてください。
